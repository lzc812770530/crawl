...
class Mymoshou5Spider(scrapy.Spider):
    name = 'mymoshou5'
    allowed_domains = ['www.dd373.com']
    start_urls = ['https://www.dd373.com/s-eja7u2-3fk9tg-d4jp31-1us7gu-0-0-jk5sj0-0-0-recycle-0-0-1-0-0-0.html']
    ...
    def parse(self, response, **kwargs):
        ...
        for ul in ul_list:
            pass
            # duration = ul.xpath('./li[@class="width185 p-l6"]/p/span[@class="font12 color666"]/text()').get() # 平均时长
            duration_time = ul.xpath(
                './li[@class="width185 p-l6"]/p/span[@class="font12 colorFF5"]/text()').get()  # 平常时长的时间
            ...
            # price = re.findall("<span class=\"font12 color666 f-left\">(.*?)</span>",rusult)  # 最高和最低收货
            ...
            print(gid, gname1, duration_time, proportion1, proportion2, quantity, price_min, price_max)
            yield MymoshouItem(
                gid=gid,
                gname=gname1,
